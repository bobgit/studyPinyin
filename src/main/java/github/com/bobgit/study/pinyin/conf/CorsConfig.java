package github.com.bobgit.study.pinyin.conf;
...
//1.编写一个支持跨域请求的 第一种存在一个问题，当服务器抛出 500 的时候依旧存在跨域问题
    @Bean
    public WebMvcConfigurer corsConfigurer(){// 类似扩张extends WebMvcConfigurerAdapter  //拦截了static静态资源 修改成implements  WebMvcConfigurer
...
/*        UrlBasedCorsConfigurationSource source = new UrlBasedCorsConfigurationSource();
        ...
        corsConfiguration.setAllowCredentials(false);//允许cookie跨域
        ...
        corsConfiguration.setAllowedMethods(Arrays.asList("GET", "HEAD", "POST","PUT", "DELETE", "OPTIONS"));//允许的方法  默认支持RequestMapping中设置的方法
        corsConfiguration.setMaxAge(3600L);// 10秒钟缓存时间300L);//缓存时间 单位秒 60*5 = 300,表示5分钟
        // 配置所有请求
